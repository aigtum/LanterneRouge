...
function checkFatigue() {
  ...
  riders.forEach((r) => {
    if (track.matrix[r.pos[0] + 1][0] != "x" && r.pos[0] < finishLineAt) {
      if (r.role == "s" && r.control === "player") {
        report += "Sprinteur received a fatigue card! ";
      }
      if (r.role == "r" && r.control === "player") {
        report += "Rouleur received a fatigue card! ";
      }
      if (r.control === "player") {
        r.deck.usedCards.push("f");
      } else {
        r.deck.deck.unshift(2);
      }
      r.deck.exhaustion += 1;
      print("Exhaustion ", r.name, r.deck.exhaustion);
    }
  });
  addToReport(report);
}
