...
class Rider {
  constructor(
    ...
    this.finished = false;
    this.deck = new Cards(cards, exhaustion);
  }
  ...
  checkBusy(pos, riders) {
    ...
    for (var i = 0; i < riders.length; i++) {
      ...
      if (
        riders[i].pos[0] == pos[0] &&
        riders[i].pos[1] == pos[1] &&
        pos[1] == 0
      ) {
  ...
  move(s, riders, track, type) {
    ...
    if (this.pos[0] + steps < track.length - 1) {
      ...
      var typeOfTiles = track.getTileTypes(
        this.pos[0],
        track.length - this.pos[0]
      );
    }
    //sout("Tile types: " + typeOfTiles);

    //sout("> " + emptyPos[0] + ", " + track.length);
    if (emptyPos[0] > track.length - 1) {
      // if the rider is ouside of the track
      //sout(">> endoftrack: rider outside of track");
      this.move(track.length - this.pos[0] - 1, riders, track);
    } else if (typeOfTiles.indexOf("u") != -1 && steps > 5) {
      // if riders pass uphill
      //sout(">> uphill: move max 5");
      this.move(5, riders, track);
    } else if (
      track.getTile(this.pos[0], this.pos[1]).type == "d" &&
      steps < 5 &&
      type != "draft"
    ) {
      // if riders are on downhill
      //sout(">> downhill: move min 5");
      this.move(5, riders, track);
    } else if (emptyPos[0] >= this.pos[0] && emptyPos[0] < track.length) {
      // if the rider is not finished
      //sout(">> normal: rider not finished");
      track.matrix[emptyPos[0]][emptyPos[1]] = "x";
      this.pos = emptyPos;
    } else if (emptyPos[0] <= this.pos[0]) {
      // if the rider is placed backwards
